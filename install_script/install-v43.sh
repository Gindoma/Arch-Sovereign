...
# FIXED: Safe validation with no set -e dependency
validate_number() {
    local input="$1"
    local min="$2"
    local max="$3"
...
# --- STANDARD SPINNER (with explicit error handling) ---
run_task() {
    local message="$1"
    local command="$2"
...
# ==============================================================================
box "" "ARCH LINUX INSTALLER V43" "$CYAN"
...
# Configure pacman (safe, no validation)
...
# Execute tasks with explicit error handling
...
# Get disk list with explicit error handling
